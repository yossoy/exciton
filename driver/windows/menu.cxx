...
bool MenuItem::InitWithItemTemplate(const picojson::value& item) {
  ...
  if (item.contains("label")) {
    label = item.get("label").get<std::string>();
  }
  ...
  if (item.contains("role")) {
    ...
    if (!roleInfo) {
      ...
      if (roleInfo->label) {
          ...
          auto appName = exciton::util::ToUTF8StringWstr(appNameW);
...
void CMenuModel::NewAppMenu(const picojson::value &argument,
  ...
  auto id = getIdFromParam(parameter);
  if (id.empty()) {
    LOG_ERROR("[%d] CMenuModel::NewAppMenu: parameter['id'] not found", __LINE__);
    Driver::Current().responceEventBoolResult(responceNo, false);
    return;
  }
  auto menuData = std::make_shared<exciton::menu::MenuData>();
...
void CMenuModel::NewPopupMenu(const picojson::value &argument,
                            const std::map<std::string, std::string> &parameter,
                            int responceNo) {
  Driver &d = Driver::Current();
  auto id = getIdFromParam(parameter);
  if (id.empty()) {
    LOG_ERROR("[%d] CMenuModel::NewPopupMenu: parameter['id'] not found", __LINE__);
    Driver::Current().responceEventBoolResult(responceNo, false);
    return;
  }
  auto menuData = std::make_shared<exciton::menu::MenuData>();
...
void newPopupMenu(const picojson::value &argument,
             const std::map<std::string, std::string> &parameter,
             int responceNo) {
  CMenuModel::Instance().NewPopupMenu(argument, parameter, responceNo);
}
...
void Menu_Init() {
  ...
  d.addEventHandler("/menu/:menu", "newApplicationMenu", newAppMenu);
  d.addEventHandler("/menu/:menu", "newPopupMenu", newPopupMenu);
